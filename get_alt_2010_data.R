...
out <- intercensal %>%
  ...
  transmute(
    ...
    Population = as.numeric(TOT_POP)   # both sexes combined
  ) %>%
  arrange(Year, Age)

dir.create("data", showWarnings = FALSE, recursive = TRUE)
write_csv(out, "data/us_intercensal_2000_2010_single_year.csv")

cat("Wrote: data/us_intercensal_2000_2010_single_year.csv (",
    nrow(out), " rows)\n", sep = "")
