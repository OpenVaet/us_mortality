# Female population evolution + horizontal bar "pyramids" by 10-year age groups
...
# ---------- 2) Female population by year + line chart ----------
totals <- pop %>%
  group_by(Year) %>%
...
p_total <- ggplot(totals, aes(x = Year, y = Population)) +
  ...
  labs(
    title = "Female population (all ages, both sexes)",
    x = NULL, y = "Population"
  ) +
...
dir.create("visual/female_pop_pyramide_2020_smoothed", showWarnings = FALSE, recursive = TRUE)

# Save total evolution chart
ggsave("visual/female_pop_total_evolution_2020_smoothed.png", p_total, width = 12, height = 7, dpi = 300)
...
for (yr in years) {
  ...
  p_pyr <- ggplot(df_y, aes(y = AgeGroup, x = Population, fill = AgeGroup)) +
    ...
    labs(
      title = paste0("Female Population by 10-year age groups — ", yr),
  ...
  ggsave(file.path("visual/female_pop_pyramide_2020_smoothed", sprintf("pyramide_%d.png", yr)),
         p_pyr, width = 9, height = h, dpi = 300)
}



cat("Saved:\n - visual/female_pop_total_evolution_2020_smoothed.png\n -", length(years), "horizontal bar pyramids in visual/female_pop_pyramide_2020_smoothed/\n")
library(magick)
library(stringr)

in_dir  <- "visual/female_pop_pyramide_2020_smoothed"
...
# Downscale & reduce colors to shrink file size
imgs_aligned <- imgs_aligned |> 
  image_scale("1200x") |>
  image_quantize(max = 128)
